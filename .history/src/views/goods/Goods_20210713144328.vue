  ...
  <div>
    ...
    <el-dialog title="添加商品信息"
      ...
      <el-form :model="addForm">
        ...
        <el-form-item label="商家名http://mrwater.oss-cn-hangzhou.aliyuncs.com/mall/20210713/5007ea40-3640-4681-99dd-08066c735350.jpg?Expires=1657694505&OSSAccessKeyId=LTAI5tDeNdt2h1bTan5NmkYK&Signature=ZcuBT%2BGP2eMCjuddDgBbeAw9j1A%3D称"
                      label-width="80px">
          <el-input v-model="addForm.seller"></el-input>
        </el-form-item>
        <el-form-item label="商品总数"
                      label-width="80px">
          <el-input v-model="addForm.saleCount"
...
export default {
  setup () {
    ...
    let imgForm = null
    const addGoodsInfo = () => {
      const af = _.cloneDeep(addForm)
      addGoods(af, imgForm).then((res) => {
        if (res.code === 200) {
    ...
    const editGoodsInfoAction = () => {
      const ef = _.cloneDeep(editForm)
      updataGoodsInfo(ef).then(res => {
        if (res.code === 200) {
